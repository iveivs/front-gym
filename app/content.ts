...
function makeTopicTasks(topic: TopicSeed): Task[] {
  ...
  return [
    ...
    task(`${topic.title}: рабочий сценарий`, topic.section, topic.level, `Смоделируй рабочий сценарий: ${topic.workExample} Реализуй минимальный код, который показывает этот сценарий без лишних абстракций.`, topic.bullets[0], "Код решает описанный рабочий сценарий и явно тренирует выбранную тему.", topic.id),
...
function makeLessonParts(topic: Pick<Topic, "title" | "section" | "focus" | "bullets" | "code" | "workExample" | "mistakes">): LessonPart[] {
  ...
  return [
    ...
    },
    {
      ...
      body: [
        `Разбей тему на маленькие куски: ${concepts.join("; ")}. Так проще учить и проще отвечать на собеседовании: не перескакиваешь с общего определения сразу к неподготовленному примеру.`,
...
function makeTopicQuestions(topic: Pick<TopicSeed, "title" | "section" | "source" | "focus" | "bullets" | "workExample" | "mistakes">): Question[] {
  ...
  return [
    q(`Что является центральной идеей темы «${topic.title}»?`, [topic.focus, "Запомнить термин без рабочего примера", "Разобрать пример без связи с текущей темой", "Только настройка редактора", "Замена всего кода одной библиотекой", "Удаление пользовательских состояний"], 0, `Центр темы: ${topic.focus}.`),
    q(`Какая подтема напрямую относится к «${topic.title}»?`, [firstConcept, "Разбор примера без связи с темой", "Сортировка файлов по названию без учебной цели", "Настройка базы данных", "Создание серверного cron", "Смена названия проекта"], 0, `${firstConcept} входит в фокус этой темы.`),
    ...
    q(`Что больше похоже на реальное применение темы «${topic.title}»?`, [topic.workExample, "Пример, который не использует фокус выбранной темы", "Переименование переменной без поведения", "Удаление всех проверок", "Хранение пароля в разметке", "Переписывание проекта без причины"], 0, topic.workExample),
    ...
    q("Что стоит сделать после чтения конспекта?", [`Написать пример с ${firstConcept} и проверить ошибочный сценарий`, "Решить упражнение, которое не тренирует текущую тему", "Считать тему закрытой по названию", "Удалить все комментарии", "Не смотреть результат", "Сразу учить следующую тему без практики"], 0, "Практика должна проверять именно текущий фокус темы."),
    q(`Какой вопрос уместен на собеседовании по теме «${topic.title}»?`, [`Когда использовать ${firstConcept}, а когда выбрать более простой подход?`, "Как выбрать инструмент без понимания задачи?", "Как настроить CI без проекта?", "Как удалить HTML?", "Как выключить браузер?", "Как не объяснять trade-off?"], 0, "Junior+ ответ должен включать ограничения и выбор инструмента."),
    q(`Как проверить, что задача по «${topic.title}» действительно относится к теме?`, [`В условии и решении явно используются ${firstConcept} или ${secondConcept}`, "В ней есть любое слово из JavaScript", "Она решает совсем другую проблему", "Она не запускается", "Она требует неизвестный backend", "Она копирует неподготовленный код"], 0, "Задача должна тренировать конкретную тему, а не соседний навык."),
    q(`Что поможет углубить тему «${topic.title}»?`, [topic.source, "Материал без примера и проверки", "Только название темы", "Минифицированный bundle", "Пустой README", "Непроверенный совет без примеров"], 0, "Источник указан как ориентир для углубления."),
...
function makeQuestions(topic: TopicSeed): Question[] {
  ...
  return [
    ...
    q(`Что чаще всего будет рабочим применением темы «${topic.title}»?`, [topic.workExample, "Смена цвета без пользовательской задачи", "Удаление всей семантики", "Отключение ошибок", "Хранение паролей в HTML", "Замена интерфейса console.log"], 0, topic.workExample),
